...
export default function Home() {
  ...
  const setAllTasks = () => setFilter("all");
  const setActiveTasks = () => setFilter("active");
  const setCompletedTasks = () => setFilter("completed");
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        <h1 className="text-4xl xl:text-6xl relative z-20 md:text-5xl font-bold text-black dark:text-white font-sans tracking-tight text-center mt-20 md:mt-40 mb-10">
          <div className="relative mx-auto inline-block w-max [filter:drop-shadow(0px_1px_3px_rgba(27,_37,_80,_0.14))]">
            ...
            <div className="relative bg-clip-text text-transparent bg-no-repeat bg-gradient-to-r from-teal-400 via-cyan-400 to-cyan-600 py-4">
              <span className="">TO-DO IN NEXT JS.</span>
            </div>
          </div>
        </h1>
        <div className="flex flex-col overflow-hidden backdrop-blur-xl bg-zinc-950/50 border-cyan-600 border-2 rounded-xl mb-5 p-4">
          <div className="flex overflow-hidden">
            <input
              ...
              className="w-full p-4 text-bolder mx-auto font-semibold focus:outline focus:border-cyan-500 border-2 caret-cyan-500 border-zinc-800 rounded-xl "
            />
          </div>
          <div className="flex overflow-hidden">
            <textarea
              ...
              className="w-full p-4 mx-auto focus:outline focus:border-cyan-500 caret-cyan-500 border-2 border-zinc-800 rounded-xl my-2"
            ></textarea>
          </div>
          <div className="flex overflow-hidden justify-between mt-2">
            <input
              ...
              className="flex overflow-hidden p-4 text-gray-400 max-w-[250px] border-2 border-zinc-800 rounded-2xl focus:outline focus:border-cyan-500"
            />
            <button
        ...
        <ul className="list-none border-cyan-600 border-2 rounded-2xl drop-shadow-lg">
          {filteredTasks.map((task) => (
            <li
              key={task.id}
              className={`flex overflow-hidden justify-between rounded-2xl hover:bg-zinc-950 items-center p-4 ${
                task.completed ? "line-through self-center" : ""
              }`}
            >
              <div className="flex items-center">
                <input
                  ...
                  className="m-4 p-2 accent-cyan-500 text-zinc-900"
                />
                <div>
              ...
              <div className="flex items-center justify-end">
                ...
                <button
                  ...
                  className="hover:text-cyan-500 text-white p-2 rounded-3xl justify-end cursor-pointer"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                ...
                <button
                  ...
                  className="hover:text-red-600 text-white p-2 rounded-3xl justify-end cursor-pointer"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
        ...
        <div className="flex justify-around m-6 gap-2">
          <button
            onClick={setAllTasks}
          ...
          <button
            onClick={() =>
              setPriority((prevPriority) => {
                if (prevPriority === null) return true;
                if (prevPriority === true) return false;
                return null;
              })
            }
            className="relative inline-flex h-12 overflow-hidden rounded-full p-[1px]"
          ...
          >
            <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#1bd4f5_0%,#009688_50%,#1bd4f5_100%)]" />
            <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-zinc-950 hover:bg-gradient-to-r from-teal-500 to-cyan-500 px-3 py-1 text-sm font-medium text-white backdrop-blur-3xl hover:text-black">
              Clear Completed
            </span>
          </button>
        </div>
